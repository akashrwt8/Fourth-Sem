...
void BFS(struct Graph *g, int startVertex) {
    ...
    while (!isEmpty(queue)) {
        ...
        {
            printf("Cycle detected at vertex %d", currentVertex);
            break;
        } else {
            // int currentVertex = dequeue(queue);
        ...
        for (int i = 0; i < g->v; ++i) {
            if (g->adj[currentVertex][i] && visited[i] == -1) {
                visited[i] = 0;
                enqueue(queue, i);
            }
        }
        ...
        for (int i = 0; i < g->v; ++i) {
            if (g->adj[currentVertex][i]) {
                if (!visited[i]) {
                    ...
                    printf("Cycle detected at edge %d-%d\n", currentVertex, i);
                    free(visited);
                    free(parent);
                    free(queue->array);
                    free(queue);
                    return 1; // Indicates a cycle is found
                }
            }
        }
    }
...
int main() {
    ...
    if (g != NULL) {
        ...
        // Check for cycle using BFS starting from vertex 0
        int cycleFound = BFS(g, 0);
        if (!cycleFound)
